...
from cal_risk.cal_risk import cal_risk_from_name
...
import sys
sys.path.append(
    r'C:\\Users\\Suriel\\Desktop\\SafeGo\\django\\safego\\cal_risk')
...
with graph.as_default():
    ...
    input_ids_p = tf.placeholder(
        tf.int32, [batch_size, FLAGS.max_seq_length], name="input_ids")
    input_mask_p = tf.placeholder(
        tf.int32, [batch_size, FLAGS.max_seq_length], name="input_mask")
    label_ids_p = tf.placeholder(
        tf.int32, [batch_size, FLAGS.max_seq_length], name="label_ids")
    segment_ids_p = tf.placeholder(
        tf.int32, [batch_size, FLAGS.max_seq_length], name="segment_ids")
...
def predict_online():
    ...
    def convert(line):
        feature = convert_single_example(
            0, line, label_list, FLAGS.max_seq_length, tokenizer, 'p')
        ...
        segment_ids = np.reshape(
            [feature.segment_ids], (batch_size, FLAGS.max_seq_length))
    ...
    with graph.as_default():
        ...
        while True:
            ...
            result = strage_combined_link_org_loc(
                sentence, pred_label_result[0], True)
            print('识别的实体有：{}'.format(''.join(result)))
            entity = '{}'.format(''.join(result))
            # 调用api获取lat lon risk
            city = "北京"
            key = 'e5927609d1b678ae42bd7d6bfb9c4687'
            lon, lat = get_location(entity, city, key)
            #risk = 0
            risk, num = cal_risk_from_name(entity, city)
            if(lon != '0'):
                answer = kb_fuzzy_classify_test(
                    entity, question, risk, lat, lon)
            else:
                answer = "您的问题暂无答案，已收录……"
                print("您的问题暂无答案，已收录……")
