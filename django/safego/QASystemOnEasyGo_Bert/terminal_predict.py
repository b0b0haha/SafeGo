...
def predict_outline():
    ...
    with graph.as_default():
        ...
        for row in nlpcc_test_data.index:
            ...
            entity = nlpcc_test_data.loc[row, "t_str"].split("|||")[0].split(">")[
                1].strip()
            attribute = nlpcc_test_data.loc[row, "t_str"].split("|||")[
                1].strip()
            ...
            # todo: 组合策略
            result = strage_combined_link_org_loc(
                sentence, pred_label_result[0], False)
            if entity in result:
                correct += 1
        ...
        print("accuracy: {}%, correct: {}, total: {}".format(
            correct*100.0/float(test_size), correct, test_size))
        print('Time used: {} sec'.format((datetime.now() - start).seconds))
